...
// Catch-all route for URLs without language prefix - redirect to default language
Route::fallback(function () {
    ...
    // Don't redirect if path already starts with a language code
    $supportedLanguages = ['en', 'fr', 'ar'];
    $pathSegments = explode('/', trim($path, '/'));
    
    if (!empty($pathSegments) && in_array($pathSegments[0], $supportedLanguages)) {
        // Already has language prefix, let it fall through to 404
        abort(404);
    }
    
    // Redirect to same path with default language prefix
...
Route::prefix('{lang}')->where(['lang' => 'en|fr|ar'])->group(function () {
    ...
    Route::middleware(['auth', 'admin'])->group(function () {
        ...
        // Public website pages (landing, support, marketing) — admin editors per page
        Route::post('/admin/website/home/save', [App\Http\Controllers\Admin\LandingHomeController::class, 'update'])
            ->name('admin.website.home.save');
        Route::post('/admin/website/home/publish', [App\Http\Controllers\Admin\LandingHomeController::class, 'publish'])
            ->name('admin.website.home.publish');
        Route::get('/admin/website/{page}', [App\Http\Controllers\Admin\WebsitePageController::class, 'show'])
            ->where('page', 'home|about|blog|contacts')
            ->name('admin.website.page');
