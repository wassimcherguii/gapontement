...
class AuthController extends Controller
{
    ...
    {
        if (Auth::check() && Auth::user()->hasAnyRole(['patient', 'companion'])) {
    ...
    public function login(Request $request)
    {
        ...
        if (! $user->hasAnyRole(['patient', 'companion'])) {
            return back()->withErrors([
                'email' => get_translation('no_patient_permission'),
            ])->withInput($request->except('password'));
        }
        ...
        return redirect()->intended(route_with_lang('patient.dashboard'));
    }
    ...
    public function logout(Request $request)
    {
        ...
        return redirect(route_with_lang('patient.login'));
    }
}
