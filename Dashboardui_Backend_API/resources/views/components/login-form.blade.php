...
<div class="w-full max-w-md space-y-8">
    ...
    <div class="text-center">
        <div class="mx-auto h-20 w-20 bg-white rounded-full shadow-lg flex items-center justify-center mb-6 floating-animation">
            <img src="{{ asset_logo() }}" alt="{{ get_logo_alt() }}" class="h-12 w-12 object-contain">
        </div>
        <h2 class="text-3xl font-bold text-gray-900 dark:text-white">
            {{ get_translation('welcome_back') }}
        </h2>
        <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">
            {{ get_translation('login_subtitle') }}
        </p>
    </div>

    <!-- Login Form Card -->
    <div class="shadow-2xl rounded-2xl p-8" style="background: var(--surface-color); border: 1px solid var(--border-color);">
        <form class="space-y-6" id="loginForm" action="{{ route_with_lang('admin.login.submit') }}" method="POST">
        @csrf
        ...
        <div class="mt-6 p-4 rounded-lg" style="background: var(--primary-color)10; border: 1px solid var(--primary-color)30;">
            <h4 class="text-sm font-semibold mb-2" style="color: var(--primary-color);">Demo Credentials:</h4>
            <div class="text-xs space-y-1 {{ is_rtl_language(app()->getLocale()) ? 'text-right' : 'text-left' }}" style="color: var(--text-secondary-color);">
                ...
                <p><strong>Companion:</strong> companion@example.com / password</p>
            </div>
        </div>
        </form>
    </div>
