...
@section('title', get_translation('edit').' '.$user->name)

@section('content')
    <h1 class="text-2xl font-semibold mb-4">{{ get_translation('edit') }}: {{ $user->name }}</h1>

    <form method="POST" action="{{ route_with_lang('admin.users.update', ['user' => $user->id]) }}" class="bg-white border rounded-lg p-4 space-y-4 max-w-xl">
        @csrf
        @method('PATCH')
        <div>
            <label class="block text-sm mb-1">{{ get_translation('name') }}</label>
            <input type="text" name="name" value="{{ old('name', $user->name) }}" class="w-full rounded-md border border-gray-300 px-3 py-2" required>
        </div>
        <div>
            <label class="block text-sm mb-1">{{ get_translation('email') }}</label>
            <input type="email" name="email" value="{{ old('email', $user->email) }}" class="w-full rounded-md border border-gray-300 px-3 py-2" required>
        </div>
        <div>
        ...
        <div>
            <label class="block text-sm mb-1">{{ get_translation('password') }} ({{ get_translation('optional_field') }})</label>
