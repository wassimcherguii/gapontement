...
@section('title', get_translation('doctor_login'))

@section('content')
<div class="min-h-screen flex items-center justify-center px-4">
    <div class="max-w-md w-full bg-white rounded-xl shadow p-6">
        <h1 class="text-2xl font-semibold mb-1">{{ get_translation('doctor_login') }}</h1>
        <p class="text-sm text-gray-600 mb-4">{{ get_translation('doctor_login_subtitle') }}</p>
        ...
        <form method="POST" action="{{ route_with_lang('doctor.login.submit') }}" class="space-y-4">
            @csrf
            <div>
            ...
            <button type="submit" class="w-full rounded-md bg-indigo-600 px-4 py-2 text-white">{{ get_translation('sign_in') }}</button>
        </form>
    </div>
</div>
@endsection
